...
def g():
    ...
    num_steps = []
    num_steps.append(("imputer", SimpleImputer(missing_values=np.nan, strategy="mean")))
    num_steps.append(("norm", StandardScaler()))
    numerical_transformer = Pipeline(steps=num_steps)

    cat_steps = []
    cat_steps.append(('imputer', SimpleImputer(missing_values="?", strategy='most_frequent')))
    ...
    svc = SVC()
    model = Pipeline(steps=[('preprocessor', preprocessor), ('svc', svc)])
